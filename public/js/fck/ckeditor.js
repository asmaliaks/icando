...
(function(){
    ...
    j.add('pastefromword',{
        ...
        n.on('paste',function(q){
            ...
            var s=window.open('',null,p,true);if(!s)return false;try{
            var t=navigator.userAgent.toLowerCase();if(t.indexOf(' chrome/18')==-1){
        ...
        readOnly:1,
        exec:function(o){
            var p,q=o.config,r=q.baseHref?'<base href="'+q.baseHref+'"/>':'',s=b.isCustomDomain();if(q.fullPage)p=o.getData().replace(/<head>/,'$&'+r).replace(/[^>]*(?=<\/title>)/,'$& &mdash; '+o.lang.preview);else{
